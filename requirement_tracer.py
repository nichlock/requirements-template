...
def write_header(f, files, hashes):
    ...
    for hashed_file in files:
        f.write("- " + hashed_file + ": " +
                md.code.inline(hashes[hashed_file]) + "\n")
    f.write("\n")
    f.write("Generated ")
    f.write(md.code.inline(str(time.strftime("%H:%M, %b %d, %Y (%z)"))))
    f.write("\n\n")


def write_table_head(f):
    f.write(
        "| ID | Title | Safety" + md.table.newline() + "Critical? | Relavent | Test Cases | Test Results |\n")
    f.write(
        "| :--- | :--- | :--- | :--- | :--- | :--- |\n")
    ...
    for line in req_f:
        ...
        if(tokens[0] == "##"):
            ...
            if(tokens[0] == "title"):
                ...
                issues = tokens[1:]
    if(len(title) == 0):
        title = md.sym.warn() + " **ERROR: REQUIREMENT NOT FOUND** " + md.sym.warn()
        criticality = safety_critical
    # print("title:", title)
...
def add_table_row(f, req_f, requirement, tests, results):
    ...
    f.write(md.table.seperator())
    # Requirement ID
    f.write(md.url.url("R-" + requirement,
                       requirements_file_name + "#R-" + requirement))
    f.write(md.table.seperator())
    # Requirement Title
    f.write(title)
    f.write(md.table.seperator())
    ...
    if criticality is safety_critical:
        f.write(md.sym.success())
    else:
        f.write(md.sym.fail())
    f.write(md.table.seperator())
    # Relavent Issues
    for i in issues:
        f.write(md.url.issue(i))
        f.write(md.table.newline())
    f.write(md.table.seperator())
    # Test Cases
    for t in tests:
        f.write("T-" + t + md.table.newline())
    f.write(md.table.seperator())
    # Tests Passed?
    for t in tests:
...
def fill_requirements_from_tap_with_tags(tap_output, req_file, out_file):
    ...
    for line in tap_output:
        ...
        if (print_stdin_to_console):
            print(f'{line}')
        tokens = tap.comments.get_tags(line)  # Splits tags out with whitespace
        if(len(tokens) == 0):
            continue
        # print(tokens)
